...
export class kCommand extends Command {
    ...
    async init(_message: Message, { args }: Arguments) {
        ...
        if (!/^\d{5}$/.test(zipcode))
            return this.Embed.error(`Invalid zip code. This command only works in the U.S. for now.`);
        else if (rad && !radius.includes(Number(rad)))
            return this.Embed.error(`Only radiuses of 1, 5, 15, 20, and 25 are allowed (default 25).`);
        ...
        if (!available || !available.providers?.length)
            return this.Embed.error(`No locations near you were found in a ${rad} mile radius.`);
