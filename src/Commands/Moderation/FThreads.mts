...
export class kCommand extends Command {
  constructor() {
    super(
      [
        `By default, Discord threads are allowed to be created by ${italic(
          'anyone'
  ...
  async init(message: Message<true>): Promise<undefined | APIEmbed> {
    const m = await message.reply({
      ...
      components: [Components.actionRow([Buttons.approve('Yes', 'approve'), Buttons.deny('No', 'deny')])]
    })
    ...
    let description = ''
    if (success.length > 0) description += `${bold('Success:')}\n`

    while (success.length !== 0 && description.length < maxDescriptionLength) {
      const { value } = success.shift()!
      const line = isCategory(value) ? `Category ${inlineCode(value.name)}\n` : `${value}\n`
