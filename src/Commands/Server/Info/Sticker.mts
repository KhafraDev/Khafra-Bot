...
import { Embed, colors, padEmbedFields } from '#khaf/utility/Constants/Embeds.mjs'

export class kCommand extends Command {
  constructor() {
    super(['Get info about a sticker!', '<sticker>'], {
      name: 'sticker',
      folder: 'Server',
      aliases: ['stickers'],
      args: [0, 0],
  ...
  async init({ stickers, guild }: Message<true>): Promise<APIEmbed> {
    if (stickers.size === 0) return Embed.error('No stickers in message! 😕')
