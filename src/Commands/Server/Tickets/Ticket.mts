...
export class kCommand extends Command {
  constructor() {
    super(['Giveaways: main place to create a type of giveaway.'], {
      name: 'ticket',
      folder: 'Server',
      aliases: ['tickets'],
      args: [0],
  ...
  init(message: Message, argument: Arguments, settings: kGuild): ReturnType<Command['init']> {
    ...
    const commandName = name.startsWith('ticket:') || name.startsWith('tickets:') ? name : `ticket:${name}`

    if (!KhafraClient.Commands.has(commandName.toLowerCase())) {
      return Embed.error(`Ticket.${name} command doesn't exist, use ${inlineCode('ticket')} for more information!`)
    }
