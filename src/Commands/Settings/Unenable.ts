...
const handle = async (
    ...
    if(inserted.modifiedCount === 1 || inserted.upsertedCount === 1) {
        ...
        Command ${command.settings.name} has been removed from exemptions ${item instanceof Role || item instanceof GuildMember ? 'for' : 'in'} ${item ?? 'the guild'}!
        `));
    } else {
...
export default class extends Command {
    constructor() {
        super(
            [
                'Re-disable a command in a guild, channel, or for a user!',
