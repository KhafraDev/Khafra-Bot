...
export default class extends Command {
    ...
    async init(message: Message, args: string[]) {
        ...
        if(!/[A-z0-9]/.test(args[0])) {
            return message.reply(this.Embed.fail(`Command must have alpha-numeric characters in it!`));
        }
        ...
        if(removeDupes.indexOf(args[0]) > -1) {
            return message.reply(this.Embed.fail('Command name is reserved.'));
        }
        ...
        if(inserted.modifiedCount === 1 || inserted.upsertedCount === 1) {
            ...
            return message.reply(this.Embed.fail('No custom commands were added!'));
        }
    }
}