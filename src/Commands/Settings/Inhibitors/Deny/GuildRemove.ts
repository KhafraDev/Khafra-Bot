...
export default class extends Command {
    ...
    async init(message: Message, args: string[], settings: GuildSettings) {
        ...
        if(!settings?.disabledGuild?.some(c => c.names.includes(command[0]))) {
            return message.channel.send(this.Embed.fail(`Command isn't disabled in this guild!`));
        }
        ...
        if(inserted.modifiedCount === 1 || inserted.upsertedCount === 1) {
            ...
            Command ${command[0]} has been re-enabled in the guild!
            `));
        } else {
