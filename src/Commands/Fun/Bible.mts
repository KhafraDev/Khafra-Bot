...
export class kCommand extends Command {
  constructor() {
    super(['Get a King James Bible verse.', 'Nahum 3:7', 'Proverbs 25:19'], {
      name: 'bible',
      folder: 'Fun',
      args: [0]
    })
  ...
  async init(_message: Message, { args, content }: Arguments): Promise<undefined | APIEmbed> {
    ...
    if (args.length === 0) {
      ...
      const book = Object.entries(titles)
    ...
    const bookAcronym = Object.entries(titles).find(([n, acr]) => n.toLowerCase() === book || acr === book)!
    ...
    if (R.BETWEEN.test(locationUnformatted)) {
      const [, chapter, ...verses] =
        R.BETWEEN.exec(locationUnformatted)! // not null since it matches the pattern
    ...
    if (R.GENERIC.test(locationUnformatted)) {
      const [, chapter, verse] = R.GENERIC.exec(locationUnformatted)!.map(Number)
