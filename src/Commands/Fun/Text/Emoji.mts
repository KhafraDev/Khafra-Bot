...
export class kCommand extends Command {
  constructor() {
    super(['Enlarge an emoji!', '🦸 🤠', '🥙', '<:Jack:579367928722489346>'], {
      name: 'emoji',
      folder: 'Fun',
      args: [1, 5],
      ratelimit: 3,
  ...
  init(message: Message<true>): string | APIEmbed {
    const unicode = parse(message.content, { assetType: 'png' }).map((e) => e.url)
    ...
    if (all.length === 0) return Embed.error('No guild or unicode emojis were in the message! 😕')
