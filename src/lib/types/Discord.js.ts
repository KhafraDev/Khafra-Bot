...
export const isTextBased = <T extends unknown>(c: T): c is T & (TextChannel | DMChannel | NewsChannel | ThreadChannel | VoiceChannel) =>
  isText(c) || isDM(c) || isThread(c) || isVoice(c)
export const isGuildTextBased = <T extends unknown>(c: T): c is T & (TextChannel | NewsChannel | ThreadChannel | VoiceChannel) =>
