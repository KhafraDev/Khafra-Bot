...
export const searchMovie = async (query: string, include_adult = false): Promise<ITMDBDetails | null> => {
  ...
  if (search.total_results === 0 || search.results.length === 0) return null
...
export const searchTV = async (query: string, include_adult = false): Promise<TVDetails | null> => {
  const params = stringify({ query, include_adult })

  const { body } = await request(new URL(`?${params}`, base.tv), { headers })
  const tv = (await body.json()) as TV

  if (tv.total_results === 0 || tv.results.length === 0) return null
