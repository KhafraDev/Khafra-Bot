...
export class kInteraction extends Interactions {
  ...
  async init(interaction: ChatInputCommandInteraction): Promise<InteractionReplyOptions | undefined> {
    ...
    const reason =
      interaction.options.getString('reason') ?? `Requested by ${interaction.user.tag} (${interaction.user.id})`
    ...
    for (let i = 1; i < 18; i++) {
      ...
      if (userOption) {
        ...
        if (member) {
          const memberPerms =
            typeof member.permissions !== 'string' ? member.permissions.bitfield : BigInt(member.permissions)
