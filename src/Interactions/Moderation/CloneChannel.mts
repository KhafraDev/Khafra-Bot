...
type CloneableChannel = TextChannel | NewsChannel | VoiceChannel
...
export class kInteraction extends Interactions {
  ...
  async init(interaction: ChatInputCommandInteraction): Promise<InteractionReplyOptions | undefined> {
    ...
    const body: RESTPostAPIGuildChannelJSONBody = {
      ...
      permission_overwrites: channel.permissionOverwrites.cache.toJSON().map((overwrite) => ({
        id: overwrite.id,
        type: overwrite.type,
    ...
    ;(await interaction.client.rest.post(Routes.guildChannels(interaction.guild.id), {
      body
    ...
    const embed = Embed.json({
      ...
      description:
        ...
        (deleteAfterwards ? ` and I am in the process of deleting ${inlineCode(channel.name)} (${channel.id})!` : '!')
    })
