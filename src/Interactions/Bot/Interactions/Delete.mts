...
export class kSubCommand extends InteractionSubCommand {
  ...
  async handle(interaction: ChatInputCommandInteraction): Promise<InteractionReplyOptions> {
    ...
    if (globally !== true) {
      ...
      await interaction.client.rest.delete(Routes.applicationGuildCommand(config.botId, config.guildId, commandId.id))
    } else {
      const commands = (await interaction.client.rest.get(
      ...
      await interaction.client.rest.delete(Routes.applicationCommand(config.botId, commandId.id))
    }
