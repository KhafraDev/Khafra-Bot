...
export class kSubCommand extends InteractionSubCommand {
  ...
  async handle(interaction: ChatInputCommandInteraction): Promise<InteractionReplyOptions | undefined> {
    ...
    const makeOptions = async (
      disable = false,
      dealer = false
    ): Promise<Pick<InteractionReplyOptions, 'embeds' | 'files' | 'components'>> => {
      ...
      const pickAce = score.sucker.some(([value, , picked]) => (value === 1 || value === 15) && !picked)
    ...
    for await (const [i] of collector) {
      if (i.customId.startsWith('ace')) {
        ...
        const cardIdx = score.sucker.findIndex(([v, , picked]) => (v === 1 || v === 15) && !picked)
  ...
  async image({ dealer, sucker }: Game, isDealer: boolean): Promise<Buffer> {
    ...
    const transformer = Transformer.fromRgbaPixels(new Uint8Array(width * height * 4), width, height)
