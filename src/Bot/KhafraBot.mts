...
export class KhafraClient extends Client {
  ...
  async loadInteractions(): Promise<typeof KhafraClient.Interactions.Commands> {
    ...
    const importPromise = interactionPaths.map((int) => import(pathToFileURL(int).href) as DynamicImportAppCommand)
    ...
    if (loaded.length !== 0) {
      ...
      if (previouslyDeployed.length === 0) {
        ...
        const overwritten = (await this.rest.put(Routes.applicationCommands(config.botId), {
          body: loadedCommands
        })) as APIApplicationCommand[]
        ...
        const deleted = existingSlashCommands.filter((c) => !loadedCommands.find((n) => n.name === c.name))

        for (const deletedCommand of deleted) {
          logger.info(`Deleting ${deletedCommand.name}!`)

          await this.rest.delete(Routes.applicationCommand(config.botId, deletedCommand.id))
        }
        ...
        for (const current of loadedCommands) {
          ...
          if (!existing || !previous) {
            ...
            const added = (await this.rest.post(Routes.applicationCommands(config.botId), {
              body: current
            })) as APIApplicationCommand
            ...
            const updated = (await this.rest.patch(Routes.applicationCommand(config.botId, existing.id), {
              body: current
            })) as APIApplicationCommand
  ...
  async startTimers(): Promise<void> {
    const timers = KhafraClient.walk('build/src/Structures/Timers', (p) => p.endsWith('.mjs'))

    const importPromise = timers.map(
      (timer) => import(pathToFileURL(timer).href) as Promise<Record<string, new (client: Client) => Timer>>
    )
    const settled = await Promise.allSettled(importPromise)
    let loadedTimers = 0
